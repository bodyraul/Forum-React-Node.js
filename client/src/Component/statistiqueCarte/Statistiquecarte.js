...
const refParamConfidentialite =useRef();
...
const [btnConfidentialite, setbtnConfidentialite] = useState(false);
...
const clickBtnConfidentialite =()=>{
    setbtnConfidentialite(!btnConfidentialite);
}
  
    useEffect(() => {
        
      if(btnConfidentialite===true){
        body.style.overflow="scroll";
        refParamConfidentialite.current.style.display = "none";
        firstBtnNav[0].style.pointerEvents ='visible';
        localStorage.setItem("confidentialite","oui");
        setconfidentialite(localStorage.getItem("confidentialite"));
      }
  ...
  useEffect(() => {
    ...
    if(!confidentialite){
        firstBtnNav[0].style.pointerEvents ='none';
  ...
  return (
    <Fragment>
        <div className='englobeStatPoker'>
            <div ref={refParamConfidentialite} className='formConfidentialité'>
                <h1 className='hformConfidentialité'>conditions générales d'utilisation et politique de confidentialité.</h1>
                <p className='pformConfidentialité'>Mon forum, utilisent des traceurs pour stocker des informations sur votre terminal. Cela me permet de vous donner accès à certaines fonctionnalités du site ou non. Vous devez accepter l'utilisation de ces traceurs si non vous ne pourrez pas naviger sur ce site. Pour d'autres information, veuillez consulter notre page <Link target='blanck' to={"/confidentialite"}>d'utilisation et de confidentialité. </Link> </p>
                <button onClick={clickBtnConfidentialite} className='bformConfidentialité'>Accepter</button>
            </div>
